  ...
  return (
    <>
      <section className="wrap--narrow" style={{ paddingTop: 32 }}>
        <nav aria-label="Breadcrumb" style={{ color: 'var(--muted)', marginBottom: 8, font: 'var(--t-caption)' }}>
          <Link href="/" style={{ color: 'inherit' }}>Home</Link>
          <span> · </span>
          <Link href="/" style={{ color: 'inherit' }}>Venue cost guides</Link>
          <span> · </span>
        ...
        <p className="muted" style={{ font: 'var(--t-body-md)', marginBottom: 16 }}>
          {v.region} · {v.capacity} · {v.tagline}
        </p>
        {sara && (
          <div className="byline" style={{ marginBottom: 24 }}>
        ...
        <div className="venue-gallery">
          <button type="button" className="venue-gallery__main" onClick={() => setLightbox(0)} aria-label="Open photos">
            <Image src={photos[0]} alt={v.name} width={800} height={480} style={{ width: '100%', height: '100%', objectFit: 'cover' }} unoptimized />
          </button>
          <div className="venue-gallery__grid">
            {[1, 2, 3, 4].map(i => (
              <button key={i} type="button" className="venue-gallery__cell" onClick={() => setLightbox(i)}
                style={{ visibility: photos[i] ? 'visible' : 'hidden' }} aria-label={`Open photo ${i + 1}`}>
                {photos[i] && <Image src={photos[i]} alt="" width={400} height={240} style={{ width: '100%', height: '100%', objectFit: 'cover' }} unoptimized />}
              </button>
            ))}
          </div>
          {photos.length > 5 && (
        ...
        {v.photosNote && <p className="muted" style={{ font: 'var(--t-caption)', marginTop: 12 }}>{v.photosNote}</p>}
      </section>
      ...
      <section style={{ padding: '64px 0 96px' }}>
        <div className="venue-grid">
          <div className="venue-body">
            ...
            <p className="muted" style={{ font: 'var(--t-caption)', marginTop: 8 }}>
              These estimates reflect the wedding day itself. Multi-day destination weddings can increase the total significantly.
            </p>
            ...
            <table className="venue-cost-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="total" style={{ textAlign: 'right' }}>€{v.estTotal80.low.toLocaleString()} – €{v.estTotal80.high.toLocaleString()}{v.id === 'son-marroig' ? '+' : ''}</td>
                </tr>
              </tbody>
            </table>
              ...
              <div className="venue-inline-photos">
                {photos.slice(5, 8).map((src, i) => (
                  <button key={i} type="button" onClick={() => setLightbox(5 + i)}>
                    <Image src={src} alt="" width={300} height={200} style={{ width: '100%', height: '100%', objectFit: 'cover' }} unoptimized />
                  </button>
                ))}
              </div>
            )}
            ...
            {alternatives.length > 0 && (
              <>
                ...
                <p>If the {v.name} estimate exceeds your budget, these venues offer comparable atmosphere at a lower total.</p>
                <div className="cost-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)', marginTop: 24 }}>
                  {alternatives.map(alt => (
                    <Link key={alt.id} href={`/venue/${alt.id}`} className="cost-card">
                      <div className="cost-card__photo">
                        <Image src={alt.img} alt={alt.name} width={400} height={250} style={{ width: '100%', height: '100%', objectFit: 'cover' }} unoptimized />
                      </div>
            ...
            <div className="faq">
              {[
                ...
                { q: 'Does the estimate include additional wedding events?', a: 'No. These estimates reflect the wedding day only. Welcome dinners, beach events and brunches can add €15–40k depending on scale.' },
          ...
          <aside>
            <div className="venue-rail">
              ...
              <span className="venue-rail__est">
                €{(v.estTotal80.low / 1000).toFixed(0)}k
                <span style={{ fontSize: 18, color: 'var(--muted)' }}> – </span>
                €{(v.estTotal80.high / 1000).toFixed(0)}k
                {v.id === 'son-marroig' && <span>+</span>}
              </span>
              <span className="venue-rail__sub">All-in: venue, catering, infrastructure, furniture, service &amp; buffer</span>
            ...
            <div style={{ marginTop: 16 }} className="venue-rail">
              ...
              <p className="muted" style={{ font: 'var(--t-body-sm)', marginBottom: 12 }}>{matchedPlanners.length} verified planners with a track record at {v.name}.</p>
...
function Lightbox({ photos, index, onClose, setIndex }: { photos: string[]; index: number; onClose: () => void; setIndex: (i: number) => void }) {
  return (
    <div className="vlb" role="dialog" aria-modal="true" onClick={onClose}>
      ...
      <Image className="vlb__img" src={photos[index]} alt="" width={1200} height={800} style={{ maxWidth: '90vw', maxHeight: '90svh', objectFit: 'contain', borderRadius: 4 }} onClick={(e) => e.stopPropagation()} unoptimized />
